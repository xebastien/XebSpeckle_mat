...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if( nrhs < 1 ) {
		...
		mexErrMsgTxt("STACKS must be of type single");
	} 
    ...
    {    
        for (oj = 0; oj < imh; oj++)
        {
            ...
            sum1 = 0;   std1 = 1;
            for (k = 0; k < imd; k++){
                sum1 += im[oi + oj*imw + k*imw*imh];
            }
            sum1 = sum1/(float)tpd;//mean
            for (k = 0; k < imd; k++){
                        std1 += pow(im[oi + oj*imw + k*imw *imh] - sum1, 2);
            }
            ...
            for (i = 0; i < tpl1; i++)
            {
                ...
                for (j = 0; j < tpl2; j++)
                {
                   ...
                   for (k = 0; k < imd; k++){
                        sum2 += tp[oi + oj*imw + i*imw*imh + j*imw*imh*tpl1 + k*imw *imh*tpl1*tpl2];
                    }
                    sum2 = sum2/(float)tpd;
    
                    for (k = 0; k < imd; k++){
                        std2 += pow(tp[oi + oj*imw + i*imw *imh + j *imw *imh *tpl1 + k* imw *imh* tpl1 *tpl2] - sum2, 2);
                    }
                    std2 = sqrt(std2/((float)tpd -1));

                    for (k = 0; k < imd; k++){
                        corrf +=  im[oi + oj*imw + k*imw *imh] * (tp[oi + oj*imw + i*imw*imh + j*imw*imh*tpl1 + k*imw *imh*tpl1*tpl2] - sum2 )/std2;            
                    }

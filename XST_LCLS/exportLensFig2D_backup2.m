...
nbstd = 2;
subplot(1,2,1)
a = mean(dphaseX(smmask(:)).*1e6);b = std(dphaseX(smmask(:)).*1e6);
imagesc(n1,m1,dphaseX.*1e6.*smmask,[a-nbstd*b a+nbstd*b])
...
subplot(1,2,2)
a = mean(dphaseY(smmask(:)).*1e6);b = std(dphaseY(smmask(:)).*1e6);
imagesc(n1,m1,dphaseY.*1e6.*smmask,[a-nbstd*b a+nbstd*b])
set(gca,'DataAspectRatio',[1 1 1]')
%set(gca,'YTick',0);set(gca,'XTick',0);set(gca,'TickLength',[0 0]);
t = colorbar('SouthOutside','peer',gca);
set(get(t,'ylabel'),'String', 'urad','FontName','Times','FontSize',14);
...
figure(4)
set(4,'units','points','position',[ 620.0000 ,73.2500, 742.0000, 476.7500])
...
%scalebar('Location','southeast','Colour',barcolor,'Unit','um')
xlabel('(mm)'); ylabel('(mm)');
title(['Wavefront error ' newline ' RMS = ' num2str(std( phi(mask1(:))*1e9 )) ' nm'] )
...
%scalebar('Location','southeast','Colour',barcolor,'Unit','um')
xlabel('(mm)'); ylabel('(mm)');
title(['Phase error ' newline ' RMS = ' num2str(std(phi(mask1(:)).*kw2)) ' rad'] )
...
%scalebar('Location','southeast','Colour',barcolor,'Unit','um')
xlabel('(mm)'); ylabel('(mm)');
title(['Lens thickness error ' newline ' RMS = ' num2str(std(thickErr(mask1(:)))) ' um'] )
...
%scalebar('Location','southeast','Colour',barcolor,'Unit','um')
xlabel('(mm)'); ylabel('(mm)');
title(['Wavefront error ' newline ' RMS = ' num2str(std( phi1(mask1(:))*1e9 )) ' nm'] )
...
title(['Phase error ' newline ' RMS = ' num2str(std(phi1(mask1(:)).*kw2)) ' rad'] )
...
%scalebar('Location','southeast','Colour',barcolor,'Unit','um')
xlabel('(mm)'); ylabel('(mm)');
title(['Thickness error , no  defoc xy ' newline ' RMS = ' num2str(std(thickErr1(mask1(:)))) ' um'] )
...
colormap(flipud(CM3))
ma1 = smmask(1:resol3D:end,1:resol3D:end);
%td = thickErr(1:9:end,1:9:end);
td = -phi1(1:resol3D:end,1:resol3D:end).*ma1;
td(td==0) = nan;
%td(~ma1) = nan;
...
subplot(1,2,1)
a = mean(phi1(mask1(:)));b = std(phi1(mask1(:)));
imagesc(phi1.*mask1.*1e9,[a-3*b a+3*b].*1e9)
colormap(flipud(CM4))
hold off
line((1:n),(1:n).*0+mil(1),'color','r')
line((1:m).*0+mil(2),(1:m),'color','g')
set(gca,'DataAspectRatio',[1 1 1]')
set(gca,'YTick',0);set(gca,'XTick',0);set(gca,'TickLength',[0 0]);


subplot(2,2,2)
plot(n1,phi(mil(1),:).*1e9.*mask1(mil(1),:),'r')
xlabel('mm');ylabel('nm')
legend('Horizontal cut')

subplot(2,2,4)
plot(m1,phi(:,mil(2)).*1e9.*mask1(:,mil(2)),'g')
xlabel('mm');ylabel('nm')
legend('Vertical cut')

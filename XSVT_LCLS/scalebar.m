...
%  - added unit string
    ...
    if scalelength==0
        sl = (max(axeslims(:,1))-min(axeslims(:,1)))*scalelengthratio;
        slorder = 10^floor(log10(sl));
        scalelength = round(sl/slorder)*slorder;    
    else
        scalelengthratio = scalelength/(max(axeslims(:,1))-min(axeslims(:,1)));%range(axeslims(:,1));
    end
    ...
    if ischar(location)
        switch location
            case 'northeast'
                anchor = [axeslims(2,1) - axesdir(1)*(max(axeslims(:,1))-min(axeslims(:,1)))*0.05, ...
                          axeslims(2,2) - axesdir(2)*(max(axeslims(:,2))-min(axeslims(:,2)))*0.05];
                direction = 'southwest';
            case 'northwest'
                ...
                direction = 'southeast';
            case 'southwest'
                anchor = [axeslims(1,1) + axesdir(1)*(max(axeslims(:,1))-min(axeslims(:,1)))*0.05, ...
                          axeslims(1,2) + axesdir(2)*(max(axeslims(:,2))-min(axeslims(:,2)))*0.05];
                direction = 'northeast';
            case 'southeast'
    ...
    udata = {'ScaleLengthRatio',scalelengthratio;...
             'AnchorRatio',[(anchor(1)-min(axeslims(:,1)))/(max(axeslims(:,1))-min(axeslims(:,1))) (anchor(2)-min(axeslims(:,2)))/(max(axeslims(:,2))-min(axeslims(:,2)))];...
             'Colour',colour;...
             'Listeners',hL;...
             'Bold',boldflag};

...
subplot(2,2,1)
p = plot(diamet(1:end), abs(focV(:,1:end)));
if size(focM,1)>5,set(p(6:end),'LineStyle','-.');end;
a = mean(abs(focV(:)));b = std(focV(:));
...
subplot(2,2,3)
p = plot(diamet(1:end), abs(focM(:,1:end)).*2.*delta.*1e6);
if size(focM,1)>5,set(p(6:end),'LineStyle','-.');end;
a = mean(abs(focM(:)).*2.*delta.*1e6);b = std(focM(:).*2.*delta.*1e6);
axis([0 diamet(1) (a-nbstd *b) (a+nbstd *b)])
legend(legendshere {:},'Location','EastOutside')%,'Orientation','horizontal') 
xlabel('Mask diameter (um)')
ylabel('Radius (um)')

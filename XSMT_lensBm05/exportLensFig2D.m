...
subplot(1,2,1)
a = 0;b = std(xg(mask1(:)));
imagesc(n1,m1,xg.*mask1 - mean(xg(mask1(:))),[a-nbstd*b a+nbstd*b]./1.3)
set(gca,'DataAspectRatio',[1 1 1]')
%set(gca,'YTick',0);set(gca,'XTick',0);set(gca,'TickLength',[0 0]);
t = colorbar('SouthOutside','peer',gca);
set(get(t,'ylabel'),'String', 'urad','FontName','Times','FontSize',14);
...
subplot(1,2,2)
%a = 0;b = std(yg(mask1(:)));
imagesc(n1,m1,yg.*mask1- mean(yg(mask1(:))),[a-nbstd*b a+nbstd*b]./1.3)
set(gca,'DataAspectRatio',[1 1 1]')
%set(gca,'YTick',0);set(gca,'XTick',0);set(gca,'TickLength',[0 0]);
t = colorbar('SouthOutside','peer',gca);
set(get(t,'ylabel'),'String', 'urad','FontName','Times','FontSize',14);
...
subplot(1,2,1)
b1 = 3*std(reshape(thephase(:,:,1),1,[]));
imagesc((~mask2).*thephase(:,:,1),[-b1 b1])
set(gca,'DataAspectRatio',[1 1 1]')
set(gca,'YTick',0);set(gca,'XTick',0);set(gca,'TickLength',[0 0]);
title(['Mask diameter = ' num2str(maskSize)]);
...
%scalebar('Location','southeast','Colour',barcolor,'Unit','um')
xlabel('(mm)'); ylabel('(mm)');
title(['T. error , no astigm & defoc- RMS = ' num2str(std(thickErr1(mask1(:)))) ' um'] )

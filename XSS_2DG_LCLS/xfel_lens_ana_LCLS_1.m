...
distSC = 1560;1726-273;%1310;     % mm  distance between detectors
% LCLS parameters
...
maskDiam = 1550./scalef;             % diameter mask size in um for second detector
...
exportPlots =  [];'/data/bm05/imaging/seb/LCLS/processing/XSS2DG/scans37-52'; 

[];%
saveEnvironment = '/data/bm05/imaging/seb/LCLS/processing/XSS2DG/'; 
enviname = 'plateDiamond';
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                           for the records                               $
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
dh1 = GradV(min(JV(bigmask==1)):max(JV(bigmask==1)) , min(JH(bigmask==1)):max(JH(bigmask==1)),1);
dh2 = GradV(min(JV(bigmask==1)):max(JV(bigmask==1)) , min(JH(bigmask==1)):max(JH(bigmask==1)),2);

dv1 = GradH(min(JV(bigmask==1)):max(JV(bigmask==1)) , min(JH(bigmask==1)):max(JH(bigmask==1)),1);
dv2 = GradH(min(JV(bigmask==1)):max(JV(bigmask==1)) , min(JH(bigmask==1)):max(JH(bigmask==1)),2);
...
GammaV  = Rv ./(Rv - distSC./1000);       GammaH  = Rh ./(Rh - distSC./1000);
GammaH(GammaH == Inf) = mean(GammaH(GammaH ~= Inf));
% GammaV = R2./R1;
% GammaH = R2./R1;

dv1 = (dv1 - mean(dv1(smmask)))./GammaV; % gamma is scaling the curvature to the detector plane
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
vert  = frankotchellappa(dh1.*smmask,dh2.*smmask);
vert  = vert - mean(vert(smmask(:)));
horiz = frankotchellappa(dv1.*smmask,dv2.*smmask);   
...
if ~isempty(saveEnvironment)
    save(fullfile(saveEnvironment,[enviname 'mat']),'phi1','phiFull','file1','raddiff1','raddiff2','maskDiam','smmask','Center');
end

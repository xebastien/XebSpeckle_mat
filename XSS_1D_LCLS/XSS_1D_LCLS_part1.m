...
fileNameH = '/data/bm05/imaging/seb/LCLS/speckle/run14_det0.h5';        % no phase plate, 1D x direction (horiz)
fileNameV = '/data/bm05/imaging/seb/LCLS/speckle/run15_det0.h5';        % no phase plate, 1D y direction (vert)
% step in that case = 0.5 um 

fileNameH = '/data/bm05/imaging/seb/LCLS/speckle/run238_det0.h5';        % with phase plate, 1D x direction (horiz)
fileNameV = '/data/bm05/imaging/seb/LCLS/speckle/run239_det0.h5';        % with phase plate, 1D y direction (vert)
% step in that case = 0.5 um 
% with grating
% fileName = '/data/bm05/imaging/seb/LCLS/speckle/run55_det2.h5';        % no phase plate step  = 
%%%%%%%%%%%%%%%%%%%
maxImages = 17490;
...
myleefilter = 20;% for th1 erosion filter (morphological processing): 0 = none, value = filter threshold
...
if interpsecim ~= 0,    [XI,YI] = meshgrid(linspace(1,n1,round(n1/interpsecim)),linspace(1,m1,round(m1/interpsecim)));  end;
...
stsampleH = zeros(m1,n1,nImages);        stsampleV = zeros(m2,n2,nImages);
for k = 1 :nImages
    stsampleH(:,:,k) = (single(files1(k).data) - mean(single(files1(k).data(:))))./std(single(files1(k).data(:)));
    stsampleV(:,:,k)    = (single(files2(k).data) - mean(single(files2(k).data(:))))./std(single(files2(k).data(:)));
end;

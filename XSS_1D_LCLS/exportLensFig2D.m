...
colormap(flipud(CM3))
ma1 = mask1(1:resol3D:end,1:resol3D:end);
%td = thickErr(1:9:end,1:9:end);
td = -phiFull(1:resol3D:end,1:resol3D:end).*ma1;
td(td==0) = nan;
%td(~ma1) = nan;
...
subplot(1,2,1)
a = mean(phi(mask1(:)));b = std(phi1(mask1(:)));
imagesc(phi.*mask1.*1e9,[a-3*b a+3*b].*1e9)
colormap(flipud(CM4))
hold off
...
subplot(2,2,2)
cutH = phi(mil(1),:).*1e9.*mask1(mil(1),:);
cutH = cutH - mean(cutH);
plot(n1,cutH,'r')
xlabel('mm');ylabel('nm')
...
subplot(2,2,4)
cutV = phi(:,mil(2)).*1e9.*mask1(:,mil(2));
cutV = cutV - mean(cutV);
plot(m1,cutV ,'g')
xlabel('mm');ylabel('nm')
legend('Vertical cut')
...
leng = min(m,n);
cutsout = cat(1,m1(1:leng),cutV',cutH  )';
